...
\subsubsection*{Parámetros}
Conjunto de productos y períodos:
\[
P=\{A,B\},\qquad T=\{1,2,3\}.
\]
...
Costos de compra:
\[
c_{A1}=10,\; c_{A2}=12,\; c_{A3}=11,\qquad c_{B1}=8,\; c_{B2}=9,\; c_{B3}=10.
\]
...
Demandas:
\[
d_{A1}=30,\; d_{A2}=40,\; d_{A3}=35,\qquad d_{B1}=20,\; d_{B2}=25,\; d_{B3}=30.
\]

Capacidad máxima por período:
\[
K=100.
\]
...
Inventario inicial:
\[
I_{A0}=0,\qquad I_{B0}=0.
\]

\subsubsection*{Variables}
\[
x_{pt}=\text{cantidad pedida del producto } p \text{ en el período } t,
\]
\[
...
\subsubsection*{Función objetivo}
\[
\min Z=\sum_{t\in T}\left(c_{At}x_{At}+c_{Bt}x_{Bt}+h_AI_{At}+h_BI_{Bt}\right)
\]

\subsubsection*{Restricciones}
Balance de inventario por producto y período:
\[
I_{A1}=I_{A0}+x_{A1}-30,\qquad
I_{A2}=I_{A1}+x_{A2}-40,\qquad
I_{A3}=I_{A2}+x_{A3}-35,
\]
\[
I_{B1}=I_{B0}+x_{B1}-20,\qquad
I_{B2}=I_{B1}+x_{B2}-25,\qquad
I_{B3}=I_{B2}+x_{B3}-30.
\]

Capacidad de almacenamiento por período:
\[
I_{At}+I_{Bt}\le 100,\qquad t=1,2,3.
\]

No negatividad:
\[
x_{pt}\ge 0,\qquad I_{pt}\ge 0,\qquad p\in\{A,B\},\; t\in\{1,2,3\}.
\]
